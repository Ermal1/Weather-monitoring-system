...
import {
  getWeatherByCity,
  weatherQueryKeys,
} from "@/app/bigdata/WeatherQueries";
...
function WeatherCard({ city }: { city: string }) {
  ...
  const getWeatherConditionColor = (condition: string) => {
    switch (condition.toLowerCase()) {
      case "clear":
      case "sunny":
        return "bg-gradient-to-r from-amber-500/20 to-yellow-500/20 text-amber-300 border-amber-400/40 backdrop-blur-sm";
      case "cloudy":
      case "partly cloudy":
        return "bg-gradient-to-r from-slate-500/20 to-gray-500/20 text-slate-300 border-slate-400/40 backdrop-blur-sm";
      case "rainy":
      case "rain":
        return "bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 border-cyan-400/40 backdrop-blur-sm";
      case "stormy":
      case "thunderstorm":
        return "bg-gradient-to-r from-purple-500/20 to-violet-500/20 text-purple-300 border-purple-400/40 backdrop-blur-sm";
      case "snowy":
      case "snow":
        return "bg-gradient-to-r from-cyan-500/20 to-teal-500/20 text-cyan-300 border-cyan-400/40 backdrop-blur-sm";
      case "foggy":
      case "fog":
        return "bg-gradient-to-r from-slate-500/20 to-zinc-500/20 text-slate-300 border-slate-400/40 backdrop-blur-sm";
      default:
        return "bg-gradient-to-r from-slate-500/20 to-gray-500/20 text-slate-300 border-slate-400/40 backdrop-blur-sm";
    }
  };
    ...
    <Card className="transition-all hover:shadow-2xl hover:shadow-emerald-500/20 hover:-translate-y-2 bg-gradient-to-br from-slate-900/80 via-slate-800/60 to-slate-900/80 backdrop-blur-xl shadow-2xl border-emerald-500/30 group">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <Badge
            variant="outline"
            className={getWeatherConditionColor(condition)}
          >
            {condition}
          </Badge>
        </CardTitle>
          ...
          {weather.temperature && (
            <div className="flex items-center justify-between pt-2 border-t border-emerald-500/20">
              ...
              <span className="font-medium text-emerald-300">
                <AnimatedCounter
                  value={Math.round(temperature + windSpeed * 0.1)}
                  duration={1000}
                />
                °C
              </span>
      ...
      <Card className="bg-gradient-to-br from-slate-900/80 to-slate-800/60 backdrop-blur-xl shadow-2xl border-rose-500/30">
        ...
        <CardContent>
          <p className="text-rose-400">
            Failed to load global weather insights
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card className="bg-gradient-to-br from-slate-900/90 via-emerald-900/20 to-teal-900/20 backdrop-blur-xl shadow-2xl border-emerald-500/30">
      <CardHeader>
        ...
        <CardDescription className="text-emerald-300/80">
          Real-time analytics from {insights.totalCitiesMonitored} cities
          worldwide
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-400/30 backdrop-blur-sm">
            ...
            <div className="text-sm text-cyan-200/80 mt-2">
              Average Temperature
            </div>
          </div>
          <div className="text-center p-4 rounded-xl bg-gradient-to-br from-rose-500/20 to-pink-500/20 border border-rose-400/30 backdrop-blur-sm">
            <div className="text-3xl font-bold text-rose-300 drop-shadow-lg">
              {insights.citiesWithAlerts}
            </div>
          ...
          <div className="text-center p-4 rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 border border-teal-400/30 backdrop-blur-sm">
            <div className="text-3xl font-bold text-teal-300 drop-shadow-lg">
              {coolestTemp}°C
            </div>
            <div className="text-sm text-teal-200/80 mt-2">
              Coolest: {insights.coolestCity?.name || "N/A"}
            </div>
          </div>
          <div className="text-center p-4 rounded-xl bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-amber-400/30 backdrop-blur-sm">
            ...
            <div className="text-sm text-amber-200/80 mt-2">
              Warmest: {insights.warmestCity?.name || "N/A"}
            </div>
          </div>
        </div>
        ...
        {insights.citiesWithAlerts > 0 && (
          <Alert className="mt-6 bg-gradient-to-r from-rose-900/40 to-orange-900/40 border-rose-500/50 backdrop-blur-sm">
            <AlertDescription className="text-rose-200">
              ⚠️ {insights.citiesWithAlerts} cities currently have severe
              weather conditions. Stay informed and take necessary precautions.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-emerald-200/80 mb-6 text-lg">
          Real-time weather monitoring across 6 major global cities with
          complete meteorological data and forecasts
        </p>
